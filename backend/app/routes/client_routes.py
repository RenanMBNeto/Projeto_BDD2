...
from app import db
from app.models import Cliente, Assessor, Conta, Portfolio, AuditoriaCompliance # <-- 1. Importar AuditoriaCompliance
...
def delete_cliente(id):
    ...
    try:
        ...
        db.session.rollback()
        return jsonify({"erro": "Erro ao deletar cliente", "detalhes": "Não é possível deletar um cliente que possui portfólios ou contas ativas."}), 400
...
def update_cliente_compliance(id):
    ...
    if status_anterior == novo_status:
        return jsonify(mensagem="Status não foi alterado.", cliente=cliente_schema.dump(cliente)), 200
